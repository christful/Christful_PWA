...
// Define the shape of a post coming from the API
interface ApiPost {
    id: string;
    content: string;
    mediaType?: string;
...
// Enriched type for the video reel display
interface VideoReel {
    id: string;
    author: string;
    authorId: string;
    ...
    likes: number;
    comments: number;
    shares: number;           // we'll keep 0 for now, could be added later
    audio: string;            // placeholder
    videoUrl: string;
    createdAt?: string;
}
...
export default function VideoPage() {
    ...
    useEffect(() => {
        if (data?.posts) {
            // Filter only posts that have a videoUrl
            const videos = data.posts
                ...
                .map(post => ({
                    ...
                    shares: 0, // not provided by API yet
                    audio: "Original Audio", // fallback
    ...
    // Reels feed using real video posts
    const ReelsFeed = () => (
            ...
            {isLoading ? (
                ...
                videoPosts.map((reel) => (
                    ...
                    >
                        ...
                        {/* Dark overlay for better text contrast (optional) */}
                        ...
                        {/* Bottom Info Overlay */}
                        <div className="absolute left-6 bottom-8 right-20 z-10">
    ...
    // Suggestions can also be populated from other video posts (limit 4)
    const Suggestions = () => {
        // Take first 4 videos that are not the current one (simplified)
            ...
            <div className="space-y-4">
                <div className="bg-white rounded-xl shadow-sm border p-5">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        {suggested.length > 0 ? suggested.map((reel, i) => (
                            <div key={reel.id} className="aspect-[9/16] bg-slate-100 rounded-lg relative overflow-hidden group cursor-pointer shadow-sm border">
                                {/* Use a placeholder image or video thumbnail - we'll use a simple gradient for now */}
